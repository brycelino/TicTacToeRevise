// TicTacToeRevise.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
/*
    a) If the computer can win immediately, it should do so
    b) If the human's next move would result in a victory,
       the AI should choose that square to block
    c) If the center square is open, choose it
    d) If any of the corner squares are open, choose one of them
...
/*static bool openCenterSquare(TicTacToe::Entry computer, TicTacToe& t)
{
    int rowCenter = 1;
    int colCenter = 1;

    if (t.board[rowCenter][colCenter] == t.EMPTY)
    {
    t.board[rowChoice][colChoice] = (char)computer;
    return false;
    }
